...
def process_symbol_group(symbols, exchange, start_datetime, end_datetime, term_days, analysis_target):
    ...
    for symbol in symbols:
        ...
        actual_file = f"./crypto_data/TraingData/AF_CSV/훈련용 데이터_24_BN/{exchange.capitalize()}_{symbol}_{analysis_target}_Actual_Frequency_{start_datetime.replace(':', '_')}_to_{end_datetime.replace(':', '_')}_{term_days}day.csv"
        
        # MAD 값 파일 경로
        mad_file = f"./crypto_data/TraingData/CSV/훈련용 데이터_24_BN/{exchange.capitalize()}_{symbol}_{analysis_target}_MAC_Results_{start_datetime.replace(':', '_')}_to_{end_datetime.replace(':', '_')}_{term_days}day.csv"
        ...
        # CSV에서 첫 번째 데이터의 `start_date` 확인
        actual_start_date = df_actual.iloc[0]["start_date"] if not df_actual.empty else start_datetime
        ...
        # **파일명에서 start_datetime을 실제 데이터의 첫 번째 start_date 값으로 변경**
        updated_start_datetime = actual_start_date.replace(":", "_")

       # 새로운 저장 파일명 생성
        output_path = f"./crypto_data/TraingData/Total_CSV/0.수집_분류전/{exchange.capitalize()}_{symbol}_{analysis_target}_Actual_Frequency_Antrophy_{updated_start_datetime}_to_{end_datetime.replace(':', '_')}_{term_days}day.csv"
