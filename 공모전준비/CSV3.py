...
def fetch_data(symbol, start_time, end_time):
    ...
    :param symbol: Trading pair symbol (e.g., 'CYBERUSDT')
...
def save_to_csv(data, symbol, save_directory):
    ...
    file_name = f"{save_directory}/{symbol.replace('USDT', '').lower()}_data_{START_DATE.replace('-', '')}_{END_DATE.replace('-', '')}.csv"
...
def main():
    ...
    # Data collection loop for the symbol (CYBERUSDT)
    for symbol in SYMBOLS:
        # Write header if file doesn't exist
        with open(f"{save_directory}/{symbol.replace('USDT', '').lower()}_data_{START_DATE.replace('-', '')}_{END_DATE.replace('-', '')}.csv", mode='w', newline='') as file:
            writer = csv.writer(file)
