...
SYMBOLS = ["BTCUSDT", "ETHUSDT", "XRPUSDT", "DOGEUSDT"]  # Bitcoin, Ethereum, Ripple, Dogecoin trading pairs
...
def save_to_csv(data, symbol):
    ...
    """
    file_name = f"{symbol.replace('USDT', '').lower()}_data_2024.csv"
...
def main():
    ...
    # Data collection loop for each symbol (BTC, ETH, XRP, DOGE)
    for symbol in SYMBOLS:
        # Write header if file doesn't exist
        with open(f"{symbol.replace('USDT', '').lower()}_data_2024.csv", mode='w', newline='') as file:
            writer = csv.writer(file)
