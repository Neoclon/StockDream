...
# Bithumb에서 데이터 가져오기
def fetch_data_bithumb(symbol, interval):
    url = f"{EXCHANGES['bithumb']}/{symbol}/{interval}"
...
def save_to_csv(data, symbol, save_directory, start_date, end_date):
    ...
    with open(file_path, mode='a', newline='') as file:
        ...
        for kline in data:
            ...
            elif isinstance(kline, dict) and 'date' in kline and 'time' in kline:  # 빗썸 데이터 구조
                timestamp = f"{kline['date']} {kline['time']}"
                writer.writerow([
                    timestamp,
                    float(kline["open"]),
                    float(kline["high"]),
...
def perform_chi_square_test(actual_frequencies, benford_dist, symbol, start_date, end_date, save_directory, digit_type):
    clean_symbol_name = clean_symbol(symbol)
    ...
    chi_square_results_path = f"{text_directory}/{clean_symbol_name}_{start_date}_{end_date}_chi_square_{digit_type.lower()}_results.txt"
    with open(chi_square_results_path, "w") as file:
        ...
        file.write(f"P-value: {p_val:.4f}\n")
        if p_val < 0.01:
            file.write(f"Conclusion: The {digit_type} digit distribution does NOT follow Benford's Law.\n")
        else:
            file.write(f"Conclusion: The {digit_type} digit distribution follows Benford's Law.\n")
...
def main():
    ...
    for symbol in symbols:
        ...
        if exchange == "binance":
            ...
            try:
                data = fetch_data_bithumb(symbol, "24H")
                if not data:
                    print(f"No data available for {symbol}.")
                    continue
                file_path = save_to_csv(data, symbol, save_directory, start_datetime, end_datetime)
        ...
        if file_path:
            ...
            try:
                ...
                if perform_all_tests == "yes":
                    perform_chi_square_test(actual_frequencies, benford_dist, symbol, start_datetime, end_datetime, save_directory, digit_choice.capitalize())
