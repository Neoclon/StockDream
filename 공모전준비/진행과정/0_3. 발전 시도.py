...
import gc
import torch
from deep_utils import detect_anomalies
...
def plot_mac_time_series(mac_values, time_labels, df, symbol, term_days, exchange, digit_type, analysis_target, start_datetime, end_datetime, anomaly_indices=None):
    """
    ...
    # 이상치 표시 (신규 추가)
    if anomaly_indices:
        anomaly_times = [time_labels[i] for i in anomaly_indices if i < len(time_labels)]
        anomaly_values = [mac_values["first"][i] for i in anomaly_indices if i < len(mac_values["first"])]
        ax1.scatter(
            anomaly_times, 
            anomaly_values,
            color='red', 
...
def perform_time_series_benford_analysis(exchange, symbols, start_datetime, end_datetime, term_days, digit_type, analysis_target):
    ...
    # 신규 딥러닝 설정
    ENABLE_DEEPLEARNING = True  # 딥러닝 활성화 플래그
    ANOMALY_THRESHOLD = 0.015   # 이상치 임계값
    ...
    for symbol in symbols:
        ...
        time_labels = []
        anomaly_indices = set()  # 이상치 인덱스 저장
        ...
        # ================================================
        # 신규 추가: MAC 시계열에 딥러닝 이상 탐지 적용
        # ================================================
        if ENABLE_DEEPLEARNING and len(mac_values["first"]) >= 20:
            try:
                anomalies = detect_anomalies(
                    mac_sequence=mac_values["first"],
                    window_size=5,
                    threshold=ANOMALY_THRESHOLD
                )
                # 이상치 인덱스 기록
                anomaly_indices = set(anomalies)
                print(f"Detected {len(anomaly_indices)} anomalies in {symbol} MAC sequence")
                
                # combined_data에 이상치 표시 업데이트
                for idx in anomaly_indices:
                    if idx < len(combined_data):
                        combined_data[idx]["is_anomaly"] = 1
            except Exception as e:
                print(f"Anomaly detection failed for {symbol}: {e}")
        ...
        if full_data and (mac_values["first"] or mac_values["second"]):
            ...
            plot_mac_time_series(
                ...
                anomaly_indices=anomaly_indices  # 신규 인자 전달
            )
