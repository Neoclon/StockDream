...
def save_to_csv(data, symbol, save_directory, start_date, end_date, exchange):
    ...
    with open(file_path, mode='a', newline='') as file:
        ...
        for kline in data:
            ...
            if isinstance(kline, dict) and 'candle_date_time_utc' in kline:
                writer.writerow([
                    kline["candle_date_time_utc"],  # UTC 시간
                    kline["opening_price"],         # 시가
                    kline["high_price"],            # 고가
                    kline["low_price"],             # 저가
                    kline["trade_price"],           # 종가
                    kline["candle_acc_trade_volume"],  # 거래량
...
def plot_benford_graph(actual_frequencies, benford_dist, symbol, start_date, end_date, save_directory, digit_type, exchange):
    clean_symbol_name = clean_symbol(symbol)
...
def plot_benford_table(actual_frequencies, benford_dist, symbol, start_date, end_date, save_directory, digit_type, exchange):
    clean_symbol_name = clean_symbol(symbol)
...
def perform_mad_test(actual_frequencies, benford_dist, symbol, start_date, end_date, save_directory, digit_type, exchange):
    clean_symbol_name = clean_symbol(symbol)
    ...
    # Save results to a text file with exchange in filename
    ...
    for symbol in symbols:
          ...
          while current_time < end_ts:
              try:
                  ...
                  if not data:
                      ...
                      break
                  # 파일 저장 시 거래소 이름 전달
                  file_path = save_to_csv(data, symbol, save_directory, start_datetime, end_datetime, exchange)
        ...
        if file_path:
            ...
            try:
                ...
                plot_benford_graph(actual_frequencies, benford_dist, symbol, start_datetime, end_datetime, save_directory, digit_choice.capitalize(), exchange)
                plot_benford_table(actual_frequencies, benford_dist, symbol, start_datetime, end_datetime, save_directory, digit_choice.capitalize(), exchange)

                if perform_all_tests == "yes":
                    perform_mad_test(actual_frequencies, benford_dist, symbol, start_datetime, end_datetime, save_directory, digit_choice.capitalize(), exchange)
