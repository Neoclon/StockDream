...
# CSV 파일 경로 설정
cryptos = ['btc', 'eth', 'xrp', 'doge', 'ada', 'avax', 'bch', 'bnb', 'dot', 
           'link', 'eos', 'ftm', 'grt', 'shib', 'sol', 'ltc', 'trx', 
           'vet', 'xlm']  # 암호화폐 리스트
...
def plot_benford_analysis(digit_counts, benford_dist, title):
    ...
    benford_table = pd.DataFrame({
        ...
        'Actual Frequency (Proportion)': [actual_frequencies_1.get(i, 0) for i in range(1, 10)],
    ...
    # 제목 추가 (암호화폐명 포함)
    plt.title(f'{crypto.capitalize()} Volume - Benford\'s Law Analysis_2024_11_12')

    # 표 이미지 저장
    image_path = f"/Users/an-youbin/Desktop/StockDream/SCV_File/Table_Image/{crypto}_benford_table.png"
...
for crypto in cryptos:
    # 데이터 읽기
    file_name = f"/Users/an-youbin/Desktop/StockDream/SCV_File/{crypto}_data_2024_11_12.csv"
...
# 결과를 CSV 파일로 저장
results_df.to_csv('/Users/an-youbin/Desktop/StockDream/SCV_File/benford_analysis_summary.csv', index=False)
